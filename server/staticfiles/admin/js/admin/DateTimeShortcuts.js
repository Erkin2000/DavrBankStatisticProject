...
'use strict';
{
    const DateTimeShortcuts = {
        ...
        // Return the current time while accounting for the backend timezone.
        now: function() {
            const serverOffset = document.body.dataset.adminUtcOffset;
            if (serverOffset) {
        ...
        addTimezoneWarning: function(inp) {
            ...
            if (timezoneOffset > 0) {
                message = ngettext(
                    'Note: You are %s hour ahead of backend time.',
                    'Note: You are %s hours ahead of backend time.',
                    timezoneOffset
                );
            }
            else {
                timezoneOffset *= -1;
                message = ngettext(
                    'Note: You are %s hour behind backend time.',
                    'Note: You are %s hours behind backend time.',
                    timezoneOffset
                );
            }
